...
<<<<<<< HEAD
    return (
        <>
            <div
                className={`fixed inset-0 z-40 bg-black/30 transition-opacity hover:cursor-pointer md:hidden ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"
                    }`}
                aria-hidden="true"
                onClick={onClose}
            />
            ...
            <aside
                className={`fixed right-0 top-0 z-50 h-full w-72 max-w-[82vw] bg-white shadow-2xl transition-transform duration-300 md:hidden ${isOpen ? "translate-x-0" : "translate-x-full"
                    }`}
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <button
                                type="button"
                                key={item}
                                className={`rounded-md border-l-4 px-4 py-3 text-left font-notoserif text-sm transition hover:cursor-pointer ${isActive
